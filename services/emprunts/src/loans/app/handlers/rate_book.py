...
class RateBook:
    ...
    def execute(self, command: RateBookCommand) -> None:
        # TODO: Optimiser les deux req de db (find_by_id et has_ever_borrowed_book): Condasser en une seule
        ...
        if not self.loan_repository.has_ever_borrowed_book(command.user_id, loan.book_id):
            raise PermissionError(
                "Vous ne pouvez pas noter un livre que vous n avez jamais emprunte.")

        if loan.borrower_id != command.user_id:
            raise PermissionError(
                "Vous ne pouvez pas noter un livre que vous n avez pas emprunte.")

        if not loan.is_returned:
            raise ValueError(
                "Vous ne pouvez noter un livre qu apres l avoir retourne.")
