...
class LivreIn(BaseModel):
    ...
    # couverture_url est absent ici : l'image est reçue en multipart et
    # uploadée vers MinIO — le token est injecté dans le service.
...
class DisponibiliteIn(BaseModel):
    """Appelé par le service Emprunts pour réserver ou retourner un livre."""
    ...
    def valider_action(cls, v: str) -> str:
        if v not in ("reserver", "retourner"):
            raise ValueError("action doit être 'reserver' ou 'retourner'.")
        return v
    ...
    def valider_quantite(cls, v: int) -> int:
        if v < 1:
            raise ValueError("quantite doit être au moins 1.")
        return v
