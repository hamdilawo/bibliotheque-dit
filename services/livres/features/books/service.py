...
async def creer_livre(data: LivreIn, file_bytes: bytes, content_type: str) -> dict:
...
async def maj_disponibilite(livre_id: UUID, action: str, quantite: int) -> dict:
    ...
    if action == "reserver":
        if livre.quantite_totale < quantite:
            from core.exceptions import StockInsuffisantException
            raise StockInsuffisantException(livre.quantite_totale, quantite)
    ...
    await livre.save()
    return {
        "message": f"Action '{action}' effectuée ({quantite} exemplaire(s)).",
        "quantite_totale": livre.quantite_totale,
        "actif": livre.actif,
    }


async def supprimer_couverture(livre_id: UUID) -> dict:
    """Supprime la couverture MinIO et efface le token en base."""
    livre = await Livre.objects().where(
        (Livre.id == livre_id) & Livre.actif.eq(True)
    ).first()
    if not livre:
    ...
    return {"livre_id": livre_id, "message": "Couverture supprimée."}
