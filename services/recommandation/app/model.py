...
class RecommendationModel:
    ...
    def preparer_emprunts(
        ...
        id_col = next((c for c in ["livre_id", "book_id"] if c in df.columns), None)
        if isbn_col is None and id_col and livre_id_to_isbn:
            df["isbn_norm"] = df[id_col].map(livre_id_to_isbn).apply(cls.normalize_isbn)
        ...
        # Emprunts sans note → score implicite basé sur le retard (signal positif = livre emprunté)
        if df["note"].isna().any():
            if "jours_retard" in df.columns:
                retard = pd.to_numeric(df["jours_retard"], errors="coerce").fillna(0)
                implicit = np.where(retard <= 0, 1.5, np.where(retard <= 7, 1.0, 0.8))
                df["note"] = df["note"].fillna(pd.Series(implicit, index=df.index))
            else:
                df["note"] = df["note"].fillna(1.0)
